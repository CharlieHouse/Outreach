...
# LMS Variables
gamma = 0.2  # 
alpha = 1e-2   #higher gets to soln faster, might overshoot
adaptive = 1
...
h = -(0.201851215295+0.0742534876181j)*10  # @ 40.3 Hz short tower
...
def updateBar():
    ...
    global micSum
    if micSum>0:
        v = np.int(np.round((np.log(micSum)+3)*3)) # +n is a shift, *y adjusts dynamic range
        v = min(20,v)
        v = max(0,v)
...
def playingCallback(in_data, frame_count, time_info, status):
    ...
    if controlState == 0:
        ...
        if adaptive ==1:
            ...
            print('u= [{},{}], error = {}'.format(u[0],u[1],sum(abs(mic_in))))
            #some logic here to catch the minima and turn adaptive off? 
